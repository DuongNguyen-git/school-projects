...
*/
    return(s->top == -1); // if s->top == -1 is true, return 1 else return 0
...
*/
    return(s->top == (STACK_SIZE - 1)); // STACK_SIZE - 1 is max top index
}
...
*/
    if(stackEmpty(s) == 1){
        stackError(SERR_TOP); // for checking if stack is empty, if its 1 -> stack is empty
    }
    else{
        *c = s->arr[s->top];
    }
...
**
*/
    if((stackEmpty(s)) == 0){ // if stack is not empty

        s->top = (s->top-1); // decrease top by 1 -> unit in top is popped
...
** funkci stackFull.
*/
	if(stackFull(s) == 1){ // if stack is full -> ERROR
        stackError(SERR_PUSH);
	}
	else{
        s->top = (s->top + 1); // increase top by 1
        s->arr[s->top] = c; // new unit with top index has value c
	}
}
