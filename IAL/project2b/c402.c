...
**/
	tBTNodePtr temp = (*RootPtr); // will go through the tree
	tBTNodePtr pom = NULL; // will be actual node
	tBTNodePtr new;

	if ((*RootPtr) == NULL){ // tree is empty -> make new node
		...
		while (temp != NULL){
			if (temp->Cont > Content){
				pom = temp;
				temp = temp->LPtr; // go to left if new value is smaller
			}
			else{
				if (temp->Cont < Content){
					pom = temp;
					temp = temp->RPtr; // go to right if it's bigger
				}
				else
					return; // if it's equal then do nothing
			}
		}
		...
		if ((new = malloc(sizeof(struct tBTNode))) != NULL){
			...
			new->LPtr = NULL;
			new->RPtr = NULL;
			if (pom->Cont > Content) // if new value is smaller then it will be in left side of the actual node
				pom->LPtr = new;
			else 
				pom->RPtr = new; // if it's bigger then in right side
		}
	}
}
/*                                  PREORDER                                  */
...
**/
	...
	while (ptr != NULL){
		BTWorkOut(ptr); // write the value
		SPushP(Stack, ptr); // push it to the stack
		ptr = ptr->LPtr; // move to next item
...
**/
	tStackP Stack;
	SInitP(&Stack); // make stack working
	Leftmost_Preorder(RootPtr, &Stack); // go through left side of the tree
	while (!SEmptyP(&Stack)){
		RootPtr = STopPopP(&Stack); // RootPtr points to the leftmost in the top of the stack and delete this from top
		Leftmost_Preorder(RootPtr->RPtr, &Stack); // now it go through right side
	}
...
**/
	...
	while (!SEmptyP(&Stack)){
		...
		BTWorkOut(RootPtr); // write the value of the actual item in the top of the stack
...
**/
	while (ptr != NULL){
		SPushP(StackP, ptr); 
		SPushB(StackB, true); // push to the top of stackB true value
		ptr = ptr->LPtr;
	}	
}
...
**/
	...
	while (!SEmptyP(&StackP)){
		RootPtr = STopPopP(&StackP); // take the leftmost from the top of the stackP and RootPtr point to it
		SPushP(&StackP, RootPtr);  // then push it in top again
		left = STopPopB(&StackB);  // take the boolean value from the top of the stackB
		if (left == true){
			SPushB(&StackB, false);
			Leftmost_Postorder(RootPtr->RPtr, &StackP, &StackB); // go to right side
		}
		else{
			RootPtr = STopPopP(&StackP); // if not then pop and write the value
			BTWorkOut(RootPtr);
		}
	}
}
...
**/
	...
	if ((*RootPtr) != NULL){
		while ( ((*RootPtr) != NULL) || (!SEmptyP(&Stack)) ){ 
			if ((*RootPtr) == NULL){  // if left side if empty
				if (!SEmptyP(&Stack))
					(*RootPtr) = STopPopP(&Stack); // receive value in right side from stack
			}
			else{
				if ((*RootPtr)->RPtr != NULL) // if right side is not empty
					SPushP(&Stack, (*RootPtr)->RPtr); // push it to the stack
				temp = (*RootPtr);
